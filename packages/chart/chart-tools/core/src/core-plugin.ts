 ...
 */
export class LineToolsCorePlugin<HorzScaleItem>
  implements ILineToolsApi, ISeriesPrimitive<HorzScaleItem>
{
  ...
  public getDataInRange(range: { from: number | string; to: number | string }): any[] {
    ...
    const fromKey =
      typeof range.from === 'number' ? range.from : this._horzScaleBehavior.key(range.from as any)
    const toKey =
      typeof range.to === 'number' ? range.to : this._horzScaleBehavior.key(range.to as any)
   ...
   */
  public getClosestBar(
    time: number | string,
    mode: 'exact' | 'floor' | 'ceil' | 'nearest',
  ): any | null {
    const targetKey = typeof time === 'number' ? time : this._horzScaleBehavior.key(time as any)
   ...
   * @returns void
   */
  public setCrossHairXY(
    x: number | null,
    y: number | null,
    visible: boolean,
    providedTime?: HorzScaleItem,
  ...
  public getLogicalPoint(x: number, y: number): LineToolPoint | null {
    return this._interactionManager.screenPointToLineToolPoint(
      new Point(x as Coordinate, y as Coordinate),
    )
  }
  ...
  private _findBarIndex(targetKey: number, mode: 'exact' | 'floor' | 'ceil' | 'nearest'): number {
    ...
    const timeScale = this._chart.timeScale()
    const lastLogical = timeScale.coordinateToLogical(
      timeScale.timeToCoordinate(lastBar.time as HorzScaleItem)!,
    )
