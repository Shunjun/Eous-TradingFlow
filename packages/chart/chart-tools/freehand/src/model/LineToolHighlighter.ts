...
export class LineToolHighlighter<HorzScaleItem> extends BaseLineTool<HorzScaleItem> {
  ...
  public constructor(
    ...
    const finalOptions = deepCopy(
      HighlighterOptionDefaults,
    ) as LineToolOptionsInternal<'Highlighter'>
  ...
  /**
   * Calculates the visibility of the freehand path based on its total bounding box.
   *
   * ### Tutorial Note on Freehand Culling
   * Freehand drawings (Brush/Highlighter) can contain hundreds of points.
   * Checking every individual segment for visibility would be extremely
   * expensive ($O(N)$).
   *
   * Instead, we use the core's Area-Based mode. This calculates the total
   * bounding box (AABB) of the entire drawing once and compares it against
   * the viewport ($O(1)$). This is mathematically equivalent to the
   * previous manual AABB calculation but much more efficient.
   *
   * @protected
   * @override
   */
  protected override updateCullingState(): void {
    ...
    // 1. Guard: Skip culling if the tool has no path yet or is being modified.
    ...
    // We pass the entire array of logical points. The core identifies the
    // min/max bounds and performs a solid 2D intersection test.
    const cullingState = getToolCullingState(
      points,
      this,
      undefined, // No horizontal extensions for standard freehand
      undefined,
      undefined,
