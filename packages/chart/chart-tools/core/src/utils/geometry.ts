...
const _seriesIntervalCache = new WeakMap<
  any,
  { direction: string; start: number; interval: number }
>()
...
function _getVerifiedBarInterval<HorzScaleItem>(
  ...
  for (let i = 0; i < maxChecks; i++) {
    ...
    const tA =
      typeof barA.time === 'string' ? convertDateStringToUTCTimestamp(barA.time) : Number(barA.time)
    const tB =
      typeof barB.time === 'string' ? convertDateStringToUTCTimestamp(barB.time) : Number(barB.time)
 ...
 */
export function logicalIndexToCoordinate(
...
export function interpolateTimeFromLogicalIndex<HorzScaleItem>(
  ...
  const firstTimeNum =
    typeof firstBar.time === 'string'
      ? convertDateStringToUTCTimestamp(firstBar.time as string)
      : Number(firstBar.time)
  const lastTimeNum =
    typeof lastBar.time === 'string'
  ...
  if (logicalIndex >= firstLogical && logicalIndex <= lastLogical) {
    ...
    const tLeft =
      typeof leftBar.time === 'string'
        ? convertDateStringToUTCTimestamp(leftBar.time as string)
        : Number(leftBar.time)
    const tRight =
      typeof rightBar.time === 'string'
        ? convertDateStringToUTCTimestamp(rightBar.time as string)
        : Number(rightBar.time)
  ...
  // --- ZONE 3: THE BLANK SPACE (Verified Extrapolation) ---
    ...
    const verifiedInterval = _getVerifiedBarInterval(
      series,
      isFuture ? 'last' : 'first',
      firstLogical,
      lastLogical,
...
export function interpolateLogicalIndexFromTime<HorzScaleItem>(
  ...
  const targetTimeNum =
    typeof timestamp === 'string' ? convertDateStringToUTCTimestamp(timestamp) : Number(timestamp)
  ...
  if (coordinate !== null) {
    ...
    if (logicalRaw !== null) {
      ...
      if (checkBar) {
        const checkTimeNum =
          typeof checkBar.time === 'string'
            ? convertDateStringToUTCTimestamp(checkBar.time)
            : Number(checkBar.time)
  ...
  if (targetTimeNum >= firstTimeNum && targetTimeNum <= lastTimeNum) {
    ...
    while (low <= high) {
      ...
      const midTimeNum =
        typeof midBar.time === 'string'
          ? convertDateStringToUTCTimestamp(midBar.time as string)
          : Number(midBar.time)
