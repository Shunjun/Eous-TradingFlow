  ...
  implements ISeriesPrimitive<HorzScaleItem>
{
  ...
  public pointToScreenPoint(point: LineToolPoint): Point | null {
    ...
    const logicalIndex = interpolateLogicalIndexFromTime(
      this._chart,
      this._series,
      point.timestamp as UTCTimestamp,
    )
  ...
  public screenPointToPoint(point: Point): LineToolPoint | null {
    ...
    if (interpolatedTime === null) {
      console.warn(
        `[BaseLineTool] screenPointToPoint: Could not determine interpolated time for screen point: ${JSON.stringify(point)}.`,
      )
      return null
    }
