...
export class InteractionManager<HorzScaleItem> {
  ...
  private readonly _boundHandleMouseLeave = (event: MouseEvent): void =>
  ...
  public screenPointToLineToolPoint(
    screenPoint: Point,
    bypassMagnet: boolean = false,
  ): LineToolPoint | null {
  ...
  public detachTool(tool: BaseLineTool<HorzScaleItem>): void {
    ...
    try {
      ...
      console.error(
        `[InteractionManager] Error detaching primitive for tool ${tool.id()}:`,
        e.message,
      )
    }
  ...
  private _getSnappedY(x: number, y: number): { y: Coordinate; price?: number } {
    ...
    const effectiveThreshold =
      toolThreshold !== undefined && toolThreshold > 0
        ? toolThreshold
    ...
    if (latestBar) {
      const latestLogical = timeScale.coordinateToLogical(
        timeScale.timeToCoordinate(latestBar.time as HorzScaleItem)!,
      )
      ...
      if (this._isDrag && isDragCreationSupported) {
        ...
        if (this._isShiftKeyDown && isShiftConstraintSupported) {
          ...
          if (originalP0 && tool.getShiftConstrainedPoint) {
            ...
            if (snapAxis === 'price') {
              constrainedScreenPoint.y = (constrainedScreenPoint.y +
                this._getActivePaneYOffset()) as Coordinate
            }
          }
        }

        // Pass Shift status to bypass magnet only on the constrained moving point
        let constrainedLogicalPoint = this.screenPointToLineToolPoint(
          constrainedScreenPoint,
          this._isShiftKeyDown,
        )
      ...
      if (this._isEditing) {
        ...
        if (isAnchorDrag) {
          ...
          if (initialP0LogicalIndex === null || newP0LogicalIndex === null) {
            console.warn(
              `[InteractionManager] Failed to determine logical indices for translation.`,
            )
            return
          }
          ...
          for (let i = 0; i < this._originalDragPoints.length; i++) {
            ...
            if (originalIndex !== null) {
              ...
              const interpolatedTime = interpolateTimeFromLogicalIndex(
                this._chart,
                this._series,
                targetLogicalIndex,
              )
              ...
              if (interpolatedTime !== null) {
                newTimestamp = this._horzScaleBehavior.key(
                  interpolatedTime as unknown as HorzScaleItem,
                ) as number
              }
            }
  ...
  private _handleMouseUp(event: MouseEvent): void {
    ...
    if (this._isCreationGesture && this._creationTool && this._mouseDownPoint) {
      ...
      if (isDiscreteClick) {
        ...
        let finalLogicalPoint: LineToolPoint | null = this.screenPointToLineToolPoint(
          finalScreenPoint,
          isConstrained,
        )
  ...
  private _handleCrosshairMove(params: MouseEventParams<HorzScaleItem>): void {
    ...
    if (
      this._plugin.getMagnetThreshold() > 0 &&
      !this._isShiftKeyDown &&
      !this._currentToolCreating
    ) {
      ...
      if (logical !== null) {
        ...
        if (interpolatedTime !== null) {
          ...
          if (pluginFormatter) {
            ...
            const internalItem =
              this._horzScaleBehavior.convertHorzItemToInternal(timeAsHorzScaleItem)
          ...
          if (snappedX !== null) {
            ...
            if (
              ...
              !this._crosshairSupplementalVisible
            ) {
              this._lastCrosshairX = snappedX
              this._lastCrosshairText = text
