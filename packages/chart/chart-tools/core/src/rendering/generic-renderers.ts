  ...
  private _getInternalData(): InternalData {
    ...
    if (!data.points || data.points.length < 2 || isDegenerate) {
      ...
      switch (rawAlignment) {
        ...
        default: {
          console.warn(
            `[TextRenderer] Unknown text alignment "${data.text?.alignment}" in fallback; defaulting to Start.`,
          )
          textStart = inflationPaddingX
    ...
    switch (rawAlignment) {
      ...
      default: {
        ...
        console.warn(
          `[TextRenderer] Unknown text alignment "${data.text?.alignment}"; defaulting to Start (padded left).`,
        )
        textX = inflationPaddingX
  ...
  private _getFontInfo(): FontInfo {
    if (this._fontInfo === null) {
      ...
      const fontStyle =
        ...
        fontSize +
        'px ' +
        ensureDefined(data.text?.font?.family)
