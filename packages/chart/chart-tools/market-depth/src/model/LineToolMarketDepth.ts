...
export class LineToolMarketDepth<HorzScaleItem> extends BaseLineTool<HorzScaleItem> {
  ...
  public constructor(
    ...
    const finalOptions = deepCopy(
      MarketDepthToolDefaultOptions,
    ) as LineToolOptionsInternal<'MarketDepth'>
  ...
  /**
   * Calculates the Market Depth tool's visibility based on its single anchor point.
   *
   * ### Tutorial Note on Market Depth Culling
   * This visualization is anchored to a specific timestamp (P0). This method
   * uses the core culling engine to determine if that anchor point is within
   * the visible range of the chart.
   *
   * Moving this check to the Model ensures that the View skips the heavy
   * lifting of processing large Bid/Ask arrays if the tool is off-screen.
   *
   * @protected
   * @override
   */
  protected override updateCullingState(): void {
    ...
    // Perform standard point culling. The tool is visible if its
    // horizontal anchor point is within the viewport buffer.
    const cullingState = getToolCullingState(points, this)

    // Mark the tool as culled if the result is not 'Visible'.
