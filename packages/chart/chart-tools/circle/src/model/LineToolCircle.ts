...
export class LineToolCircle<HorzScaleItem> extends BaseLineTool<HorzScaleItem> {
  ...
  public override setPoint(index: number, point: LineToolPoint): void {
    ...
    if (index === 0) {
      ...
      const newRadiusLogical = Math.sqrt(
        Math.pow(point.timestamp - center.timestamp, 2) + Math.pow(point.price - center.price, 2),
      )
      ...
      const originalRadius = Math.sqrt(
        Math.pow(originalVector.timestamp, 2) + Math.pow(originalVector.price, 2),
      )
  ...
  protected override updateCullingState(): void {
    ...
    if (
      this.getPermanentPointsCount() < this.pointsCount ||
      this.isCreating() ||
      this.isEditing()
    ) {
