...
export class LineToolMarketDepthPaneView<HorzScaleItem> extends LineToolPaneView<HorzScaleItem> {
  ...
  protected override _updateImpl(height: number, width: number): void {
     ...
     * CULLING CHECK
     *
     * We query the Model's pre-calculated state. If the tool is off-screen,
     * we exit immediately to avoid deep-copying and processing the
     * Bid/Ask data arrays.
     */
    ...
    const rawData = deepCopy(options.marketDepth.marketDepthData) // Screen coordinates of the single anchor P0

    // --- DYNAMIC OFFSET CALCULATION START ---

    // Access the TimeScale options to find the current barSpacing.
    // barSpacing is the pixel distance between the center of two candles.
    const timeScale = this._chart.timeScale()
    const timeScaleOptions = timeScale.options()
    const barSpacing = timeScaleOptions.barSpacing

    // Calculate halfBarWidth to find the distance from the center to the candle edge.
    const halfBarWidth = barSpacing / 2

    // Create dynamic options that add the candle width to the user's base offset.
    const dynamicDepthOptions = {
      ...options.marketDepth,
      timestampStartOffset: (options.marketDepth.timestampStartOffset || 0) + halfBarWidth,
    }

    // Also adjust the anchor label offset so it remains aligned with the bars.
    const dynamicTextOptions = deepCopy(options.text)
    if (dynamicTextOptions.box?.offset) {
      dynamicTextOptions.box.offset.x += halfBarWidth
    }

    // --- DYNAMIC OFFSET CALCULATION END ---
    ...
    const marketDepthRendererData: MarketDepthRendererData = {
      ...
      options: dynamicDepthOptions,
      textOptions: dynamicTextOptions, // <-- FIX: Pass the text options to the renderer data
